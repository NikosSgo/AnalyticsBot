...
ADVANCED_QUERY_DEFINITIONS: Dict[str, QueryDef] = {
    "count_videos_top_views_by_date_range": {
        ...
        "sql": "SELECT COUNT(*) FROM (SELECT video_id FROM video_snapshots WHERE ($1 IS NULL OR created_at >= $1) AND ($2 IS NULL OR created_at < $2) AND delta_views_count > 0 ORDER BY delta_views_count DESC LIMIT 10) t;",
        "parameters": {
    ...
    "count_active_creators_by_date_range": {
        ...
        "sql": "SELECT COUNT(DISTINCT v.creator_id) FROM videos v JOIN video_snapshots s ON v.id = s.video_id WHERE ($1 IS NULL OR s.created_at >= $1) AND ($2 IS NULL OR s.created_at < $2) AND s.delta_views_count > 0;",
        "parameters": {
            "date_from": {"type": "string", "description": "Дата начала (YYYY-MM-DD или ISO datetime)"},
            "date_to": {"type": "string", "description": "Дата окончания (YYYY-MM-DD или ISO datetime)"},
            "date": {"type": "string", "description": "Конкретная дата (YYYY-MM-DD или ISO datetime)"},
