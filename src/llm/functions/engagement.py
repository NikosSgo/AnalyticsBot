...
ENGAGEMENT_QUERY_DEFINITIONS: Dict[str, QueryDef] = {
    "count_videos_likes_gt": {
        ...
        "sql": "SELECT COUNT(*) FROM videos WHERE likes_count > $1 AND ($2 IS NULL OR video_created_at >= $2) AND ($3 IS NULL OR video_created_at < $3);",
        "parameters": {
    ...
    "count_videos_comments_gt": {
        ...
        "sql": "SELECT COUNT(*) FROM videos WHERE comments_count > $1 AND ($2 IS NULL OR video_created_at >= $2) AND ($3 IS NULL OR video_created_at < $3);",
        "parameters": {
    ...
    "sum_delta_likes_by_date": {
        ...
        "sql": "SELECT COALESCE(SUM(delta_likes_count), 0) FROM video_snapshots WHERE ($1 IS NULL OR created_at >= $1) AND ($2 IS NULL OR created_at < $2);",
        "parameters": {
            "date_from": {"type": "string", "description": "Дата начала (YYYY-MM-DD или ISO datetime)"},
            "date_to": {"type": "string", "description": "Дата окончания (YYYY-MM-DD или ISO datetime)"},
            "date": {"type": "string", "description": "Конкретная дата (YYYY-MM-DD или ISO datetime)"},
